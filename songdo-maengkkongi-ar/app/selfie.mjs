...
export class Selfie{
 constructor(host,notify,{reduced=false}={}){
  this.host=host;this.notify=notify;this.reduced=reduced;this.dead=false;this.active=false;this.tracked=false;this.manual=false;this.side=11;this.lastSeen=0;this.position={x:.73,y:.62};
 ...
 async start(){
  ...
  this.stream=stream;this.video.srcObject=stream;await this.video.play();this.active=true;this.notify('어깨를 찾고 있어요. 얼굴과 양쪽 어깨가 보이게 해주세요.');
  try{
   if(this.pose)return;
  ...
  }catch(e){if(!this.dead){this.useManual();this.notify('자동 추적을 사용할 수 없어 직접 놓기로 전환했어요. 어깨 위치를 눌러주세요.');}}
 }
 ...
 async capture(){
  if(!this.active||!this.video.videoWidth)throw Error('카메라를 먼저 켜주세요.');
  ...
  const ctx=c.getContext('2d'),w=c.width,h=c.height-140,vw=this.video.videoWidth,vh=this.video.videoHeight,scale=Math.max(w/vw,h/vh);
  ctx.save();ctx.translate(w,0);ctx.scale(-1,1);ctx.drawImage(this.video,(w-vw*scale)/2,(h-vh*scale)/2,vw*scale,vh*scale);ctx.restore();
 ...
 stopCamera(){this.active=false;this.stream?.getTracks().forEach(t=>t.stop());this.video.srcObject=null;}
 dispose(){this.dead=true;this.stopCamera();this.pose?.close();this.pose=null;this.renderer.setAnimationLoop(null);this.resizeObserver.disconnect();this.host.removeEventListener('pointerdown',this.click);this.renderer.dispose();this.video.remove();this.renderer.domElement.remove();this.mascot.traverse(m=>{if(m.isMesh)m.geometry.dispose();});}
}
